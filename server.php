...
function saveProfile($data) {
    ...
    if (isset($_SESSION['user'])) {
        $stmt = $db->prepare("UPDATE users SET workplace = ?, position = ?, education = ?, experience = ?, publications = ?, projects = ?, patents = ? WHERE id = ?");
        $stmt->bind_param("sssssssi", $data['workplace'], $data['position'], $data['education'], $data['experience'], $data['publications'], $data['projects'], $data['patents'], $_SESSION['user']['id']);
        return $stmt->execute();
    } else {
        return false;
    }
